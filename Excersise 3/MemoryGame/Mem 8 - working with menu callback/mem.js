...
app.controller("AppCtrl", ['$scope', '$timeout', 'ENUMS', 'FactoryCards', function ($scope, $timeout, ENUMS ,FactoryCards){
		$scope.data = {};
		$scope.data.lastSelectedIndex = null;
		$scope.data.lastSelectedScope = null;
		$scope.menu = angular.copy(ENUMS.Providers);

		$scope.changeGame = function(providerType){
		...
		//(ENUMS.Providers.Numbers.id);
		$scope.changeGame(ENUMS.Providers.Names.id);
