...
app.controller("AppCtrl", ['$scope', 'ENUMS', 'FactoryCards','$timeout', function ($scope, ENUMS ,FactoryCards, $timeout){
		...
		$scope.data.changeGame = function(providerType){
			...
			$timeout(function(){
				//we need this timeout as a workaround to solve the bug "Having all the cards already flipped to answer"
				//This is cause we need the digest() to finished first in order the UI will be created and only than set the TweenLite.set('.cardWrapper'... CSS
